  ...
  deploy:
    ...
    steps:
      ...
      - name: Deploy to production
        run: |
          echo "🚀 Deploy Docker container to production"
...
export async function generateDevOpsInfrastructure(projectDir: string, config: ProjectConfig): Promise<void> {
  ...
  console.log('✅ DevOps infrastructure generated');
}
...
export function validateDevOpsConfig(config: ProjectConfig): string[] {
  ...
  if (config.docker && config.deployTarget === 'vercel') {
    warnings.push('⚠️  Docker configuration not needed for Vercel deployment');
  }

  if (!config.docker && config.deployTarget === 'docker') {
    warnings.push('⚠️  Docker deployment target requires Docker configuration');
  }

  if (config.cicd === 'none' && config.deployTarget !== 'manual') {
    warnings.push('⚠️  Automated deployment requires CI/CD configuration');
  }
